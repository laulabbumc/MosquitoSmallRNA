...
# katia
#DATABASE_PATH=/projectnb/lau-bumc/gdayama/projects/yamashita_Dmel_github_test/reference
DATABASE_PATH=$refdir
echo "Local Reference directory $refdir"
echo "Flag: $flag"
...
for (( i=1; i<=$n_sample; i++ ))
do
  bash align_to_repeat_virus.sh ${a[$i]} $organism $DATABASE_PATH $flag
done
